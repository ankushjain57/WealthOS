...
export default function Dashboard({ onNetWorthUpdate }) {
  ...
  useEffect(() => {
    Promise.all([api.getSummary(), api.getSectors(), api.getConcentration(), api.getBuckets()])
      .then(([s, sec, top, bk]) => {
        ...
        setTopH(top);
        setBuckets(bk);
        // Use snapshot net_worth for header (investments + cash + real estate)
        const nw = parseFloat(s.net_worth || s.total_value || 0);
  ...
  // Use snapshot investments value if available, else fall back to holdings sum
  const totalInv      = parseFloat(summary?.investments || summary?.total_value || 0);
  const totalHoldings = parseFloat(summary?.total_value || 0);
  const cash          = parseFloat(summary?.cash || 0);
  const netWorth      = parseFloat(summary?.net_worth || totalInv);
    ...
    <div>
      ...
      <div className="g4 mb18">
        <div className="kpi">
          ...
          <div className="kpi-val">{fmt.dollar(netWorth)}</div>
          <div className="kpi-sub">Investments + cash + real estate</div>
        </div>
        <div className="kpi">
          <div className="kpi-lbl">Investments</div>
          <div className="kpi-val">{fmt.dollar(totalInv)}</div>
          <div className="kpi-sub">{summary?.position_count} positions</div>
        </div>
        <div className="kpi">
          <div className="kpi-lbl">Today's P&amp;L</div>
          <div className={`kpi-val ${dayPL >= 0 ? 'pos' : 'neg'}`}>{fmt.sign(dayPL)}</div>
          <div className="kpi-sub">Across all holdings</div>
        </div>
        <div className="kpi">
          <div className="kpi-lbl">Cash &amp; Savings</div>
          <div className="kpi-val">{fmt.dollar(cash)}</div>
          <div className="kpi-sub">Bank accounts</div>
        </div>
      </div>

      <div className="g2 mb18">
        <div className="kpi">
          <div className="kpi-lbl">Taxable</div>
          <div className="kpi-val">{fmt.dollar(bucketMap['Taxable'] || 0)}</div>
          <div className="kpi-sub">{fmt.pct((bucketMap['Taxable'] || 0) / totalAccounts * 100)} of accounts</div>
        </div>
        <div className="kpi">
          <div className="kpi-lbl">Tax-Deferred (IRA / 401k)</div>
          <div className="kpi-val">{fmt.dollar(bucketMap['Tax-Deferred'] || 0)}</div>
          <div className="kpi-sub">IRA + 401k</div>
        </div>
      </div>
      ...
      <div className="mb18">
        ...
        <div className="alert a-blue"><span>📊</span><div><strong>Idle Cash:</strong> {fmt.dollar(cash)} in bank accounts. T-bills yield ~5.2% — deploying adds ~$40K/yr risk-free.</div></div>
      </div>
            ...
            <Doughnut
              ...
              options={{
                ...
                plugins: {
                  ...
                  tooltip: { callbacks: { label: c => ` ${fmt.pct(c.raw / totalHoldings * 100)} — ${fmt.dollar(c.raw)}` } }
                }
              }}
            />
          </div>
      ...
      <div className="card">
        ...
        <div className="tbl-wrap">
          <table>
            <thead>
              <tr><th>Ticker</th><th>Name</th><th className="r">Value</th><th className="r">Weight</th></tr>
            </thead>
            <tbody>
              {topH.map((h, i) => (
                <tr key={i}>
